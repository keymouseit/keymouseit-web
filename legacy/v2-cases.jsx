// KeyMouse IT V2 — Featured case studies. One large story + two supporting.

function FlowStep({ kind, text, accent, last }) {
  const meta = { Challenge: { i: "CircleAlert,AlertCircle", c: "#E11D48", bg: "#FEF2F2" }, Solution: { i: "Lightbulb,Wrench", c: accent, bg: withAlpha(accent, 0.1) }, Outcome: { i: "TrendingUp", c: "var(--green)", bg: "var(--green-50)" } }[kind];
  return (
    ...
    <Reveal>
      <div className="card feat-case" style={{ padding: 0, overflow: "hidden", display: "grid", gridTemplateColumns: "1.25fr 0.75fr", boxShadow: "var(--sh-lg)" }}>
        ...
        <div style={{ background: "linear-gradient(160deg,#0B1120,#131A2C)", padding: "38px 36px", display: "flex", flexDirection: "column", justifyContent: "center", gap: 26, position: "relative", overflow: "hidden" }}>
          ...
          <a className="btn btn-primary" href="#contact" style={{ position: "relative", marginTop: 8 }}>View Case Study <Arrow /></a>
        </div>
      </div>
    </Reveal>
  );
    ...
    <section className="section" id="cases">
      <div className="container">
        ...
        <div className="cases-supp" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 22, marginTop: 22 }}>
          <SupportingCase c={C[1]} i={0} />
          <SupportingCase c={C[3]} i={1} />
        </div>
      </div>
    </section>
  );
