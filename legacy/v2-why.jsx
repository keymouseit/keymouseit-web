// KeyMouse IT V2 — "Why KeyMouse" scroll-driven transformation comparison.
...
function CompareRow({ row, i, header }) {
  ...
  return (
    <Reveal delay={i * 90}>
      <div className="cmp-r" style={{ display: "flex", alignItems: "stretch", borderTop: "1px solid var(--line)" }}>
        {/* agency */}
    ...
    <section className="section" id="why" style={{ position: "relative", overflow: "hidden", background: "linear-gradient(180deg,#FFFFFF,#F6F8FE)" }}>
      ...
      <div className="container" style={{ position: "relative" }}>
        <Reveal><SectionHead center maxWidth={700}
          ...
          lead="We help companies build operational systems that create measurable business outcomes — not just ship features." /></Reveal>
        ...
        <Reveal delay={80}>
          <div className="card cmp-card" style={{ maxWidth: 880, margin: "36px auto 0", overflow: "hidden", boxShadow: "var(--sh-xl)" }}>
            <CompareRow header />
            {window.WHY_COMPARE.map((row, i) => <CompareRow key={row.us} row={row} i={i} />)}
          </div>
        </Reveal>
