...
function StagePanel({ s, i, open, onToggle }) {
  return (
    <Reveal delay={i * 110} className="cap-panel" style={{ display: "flex", flex: 1 }}>
      <div className="card" style={{ padding: 0, overflow: "hidden", height: "100%", display: "flex", flexDirection: "column", width: "100%" }}>
      ...
      <div className="container">
        <Reveal>
          <SectionHead center maxWidth={700}
            ...
            lead="One partner across the entire lifecycle — from strategy to scale. No hand-offs, no gaps." />
        </Reveal>
          ...
          {CAP_STAGES.map((s, i) => (
            <React.Fragment key={s.stage}>
              <div style={{ flex: 1, display: "flex" }}>
                <StagePanel s={s} i={i} open={open === i} onToggle={() => setOpen(open === i ? -1 : i)} />
              </div>
