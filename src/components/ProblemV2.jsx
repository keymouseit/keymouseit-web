...
const TOOLS = [
  ...
  { t: "ERP", i: "Layers" }, 
  { t: "Email", i: "Mail" },
  { t: "Spreadsheets", i: "Table,Sheet" }, 
  { t: "Inventory", i: "Package" }, 
  { t: "Documents", i: "FileText" },
];
    ...
    <div style={{ position: "relative", width: "100%", aspectRatio: "1 / 1", maxWidth: 480, margin: "0 auto" }}>
      ...
      <svg viewBox="0 0 100 100" width="100%" height="100%" style={{ position: "absolute", inset: 0 }}>
        ...
        <g style={{ opacity: connected ? 1 : 0, transition: "opacity 0.5s var(--ease) 0.15s" }}>
          {TOOLS.map((_, i) => { 
            const p = ring(i); 
            return <line key={i} x1="50" y1="50" x2={p.x} y2={p.y} stroke="rgba(37,99,255,0.5)" strokeWidth="0.5" strokeDasharray="1.4 1.4" className={connected ? "flow-line" : ""} />; 
          })}
        </g>
      </svg>
